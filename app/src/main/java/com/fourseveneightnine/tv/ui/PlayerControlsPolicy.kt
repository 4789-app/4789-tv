...
internal object PlayerControlsPolicy {
    ...
    fun trackLabel(
        ...
        val readableName = name.trim().takeIf { it.isNotEmpty() && !it.equals(language, true) }
        if (readableLanguage != null) parts += readableLanguage
        if (readableName != null) parts += readableName
        codec?.substringAfter('/')?.uppercase(Locale.US)?.takeIf(String::isNotEmpty)?.let { parts += it }
        channels?.takeIf { it > 0 }?.let { parts += "${it}ch" }
