...
If a future build drops NextLib entirely — playing only through Media3's own decoders and the
LGPL mpv path — the copyleft trigger goes with it and the licence can be revisited. Until then,
GPL-3.0 is the correct and required label.
...
| libmpv for Android (`dev.jdtech.mpv:libmpv`) | 1.0.0 | MIT wrapper around LGPL mpv | Notice only; mpv itself is LGPL-family |
