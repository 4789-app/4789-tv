...
verify_box() { # verify_box <serial-or-"-"> <host>
  ...
  if [[ $adb_ok -eq 1 ]]; then
    ...
    if [[ "$engine" == "mpv" ]]; then
      ...
      # Read the complete bounded diagnostics file. `tail -40` can discard the first-frame event
      # when a fresh-player retry or surround probe follows the baseline playback.
      local diagnostics
      diagnostics="$(adb -s "$serial" shell "run-as com.fourseveneightnine.tv cat files/receiver-diagnostics.log 2>/dev/null" 2>/dev/null)"
      frames="$(grep -c 'exo.firstFrameRendered' <<<"$diagnostics")"
    ...
    local track
    track="$(grep 'exo.audio.trackInit' <<<"$diagnostics" | tail -1)"
    if [[ -z "$track" ]]; then
    ...
    # Pair output events by AudioTrack session. A player retry can emit a short-lived start/stop
    # before the successful player, so comparing the last start to the last stop can report 0s or
    # mix two different sessions. If no complete pair is present, the receiver diagnostics plus
    # the playback clock remain the portable evidence.
    local held
    held="$(python3 -c 'import re, sys
from datetime import datetime
starts = {}
durations = []
for line in sys.stdin:
    match = re.search(r"\s(\d{2}:\d{2}:\d{2}\.\d{3})\s.*(startOutput|stopOutput)\(\).*session (\d+)", line)
    if not match:
        continue
    stamp, event, session = match.groups()
    value = datetime.strptime(stamp, "%H:%M:%S.%f")
    if event == "startOutput":
        starts[session] = value
    elif session in starts:
        durations.append((value - starts.pop(session)).total_seconds())
print(round(max(durations), 2) if durations else "")' <<<"$log")"
    if [[ -z "$held" ]]; then
      info "platform APM logs unavailable on this device — sink evidence above stands"
    elif python3 -c "import sys; sys.exit(0 if float('$held') >= $MIN_AUDIO_SECONDS else 1)"; then
      pass "audio output held ${held}s"
    elif [[ "${secs:-0}" -gt 3 ]]; then
      # play_and_measure intentionally leaves the successful player running while this block
      # inspects it. A short start/stop pair can belong to ExoPlayer's discarded retry, while the
      # current player has already advanced. Do not call that a route collapse without a matching
      # long-lived stop event for the active playback.
      pass "audio output active while playback advanced to ${secs}s (no long-lived stop event)"
    else
