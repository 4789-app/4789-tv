 ...
 */
internal class KodiTransport(
    dispatcher: RpcDispatcher,
    private val events: Flow<ReceiverEvent>,
    private val settingsPairingEndpoint: SettingsPairingEndpoint? = null,
    ...
    /** First-party UI events that are not part of the player engine's generic ReceiverEvent flow. */
    suspend fun notifyLocalControl(action: String, snapshot: com.fourseveneightnine.tv.protocol.ReceiverSnapshot) {
        clients.broadcast(KodiNotifications.encodeLocalControl(action, snapshot))
    }

    suspend fun notifyPlaybackStatus(
        status: String,
        message: String,
        snapshot: com.fourseveneightnine.tv.protocol.ReceiverSnapshot,
    ) {
        clients.broadcast(KodiNotifications.encodePlaybackStatus(status, message, snapshot))
    }
            ...
            serverConfig {
                ...
                module {
                    ...
                    routing {
                        ...
                        settingsPairingEndpoint?.let { endpoint ->
                            post("/x4789/pair/{pairingID}") {
                                val pairingID = call.parameters["pairingID"].orEmpty()
                                val body = receiveBoundedBody(call, MAX_SETTINGS_REQUEST_BYTES) ?: return@post
                                respondSettings(call, endpoint.stagePair(pairingID, body))
                            }
                            post("/x4789/pair/{pairingID}/catalog") {
                                val pairingID = call.parameters["pairingID"].orEmpty()
                                val body = receiveBoundedBody(call, MAX_CATALOG_REQUEST_BYTES) ?: return@post
                                respondSettings(call, endpoint.stagePairCatalog(pairingID, body))
                            }
                            get("/x4789/pair/{pairingID}/status") {
                                val pairingID = call.parameters["pairingID"].orEmpty()
                                respondSettings(call, endpoint.pairStatus(pairingID))
                            }
                            post("/x4789/sync/{receiverID}") {
                                val receiverID = call.parameters["receiverID"].orEmpty()
                                val body = receiveBoundedBody(call, MAX_SETTINGS_REQUEST_BYTES) ?: return@post
                                respondSettings(call, endpoint.applySync(receiverID, body))
                            }
                        }
    ...
    private suspend fun receiveBoundedBody(call: ApplicationCall, maximumBytes: Int): String? {
    ...
    private suspend fun respondSettings(call: ApplicationCall, response: SettingsEndpointResponse) {
    ...
    private companion object {
        ...
        const val MAX_SETTINGS_REQUEST_BYTES = SettingsPairingCrypto.MAX_ENVELOPE_BYTES
        const val MAX_CATALOG_REQUEST_BYTES = SettingsPairingCrypto.MAX_CATALOG_ENVELOPE_BYTES
