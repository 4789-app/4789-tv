...
class ReceiverEnginePolicyTest {
    ...
    @Test
    fun unsafeMpvOverrideFallsBackToExo() {
        assertEquals(ReceiverEngine.Exo, ReceiverEnginePolicy.engine("Amazon", "AFTKA", "mpv"))
    ...
    @Test
    fun onlySelectableValuesNormalise() {
        assertNull(ReceiverEnginePolicy.normalizedOverride(" MPV "))
    ...
    fun autoIsStoredAsNoOverrideSoALaterDefaultChangeIsPickedUp() {
        ...
        assertNull(ReceiverEnginePolicy.storedValue("mpv"))
        assertEquals("exo", ReceiverEnginePolicy.storedValue("exo"))
    }
}
