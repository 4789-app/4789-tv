...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def authorized(self) -> bool:
        ...
        supplied = (query.get("k", [""])[0]) or self.headers.get("X-Installer-Key", "")
    ...
    def do_GET(self):  # noqa: N802
        ...
        if route in ("/", "/index.html"):
            ...
            self._send(MOBILE_PAGE.replace("__KEY__", TOKEN).encode(), "text/html; charset=utf-8")
        elif route == "/icon.png":
...
<link rel="apple-touch-icon" href="/icon.png?k=__KEY__">
...
<script>
  ...
  $("check").onclick = () => call("/check", {}, $("check"));
  $("sideload").onclick = () => {
  ...
  document.querySelectorAll(".tile").forEach((tile) => {
    tile.onclick = () => {
      ...
      if (act === "install") call("/install", {}, tile);
