...
You need JDK 17. Not 21, not 24 — the receiver targets the Fire OS 7 runtime, and a newer JDK
changes Gradle's toolchain resolution until the build stops matching what ships.

```bash
JAVA_HOME=/opt/homebrew/opt/openjdk@17 ./gradlew clean testDebugUnitTest lintDebug assembleDebug
```

That command is the gate. CI runs exactly it, and a pull request that does not pass it locally
